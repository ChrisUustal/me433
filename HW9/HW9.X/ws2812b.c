...
#define LOWTIME 15 // number of 48MHz cycles to be low for 0.35uS ORIGINALLY 15
#define HIGHTIME 65 // number of 48MHz cycles to be high for 1.65uS ORIGINALLY 65
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].r & (0b1 << j)/* identify the bit in c[].r, is it 1 */) {
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].g & (0b1 << j)/* identify the bit in c[].r, is it 1 */) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].b & (0b1 << j)/* identify the bit in c[].r, is it 1 */) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
