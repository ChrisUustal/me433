...
//        sprintf(m_out, "debug1\r\n");
//        WriteUART1(m_out);
        
        ReadUART1(m_in,100); // wait for a newline
        // don't actually have to use what is in m
        
//        sprintf(m_out, "debug2\r\n");
//        WriteUART1(m_out);
        
        // collect data
        i = 0;
        burst_read_mpu6050(IMU_buf);
        ax[0] = conv_xXL(IMU_buf);
        ay[0] = conv_yXL(IMU_buf);
        az[0] = conv_zXL(IMU_buf);
        gx[0] = conv_xG(IMU_buf);
        gy[0] = conv_yG(IMU_buf);
        gz[0] = conv_zG(IMU_buf);
        temp[0] = conv_temp(IMU_buf);
        ...
        for (i=1; i<NUM_DATA_PNTS; i++){
            ...
            // read IMU
            burst_read_mpu6050(IMU_buf);
            ax[i] = alpha*conv_xXL(IMU_buf) + (1-alpha)*ax[i-1];
            ay[i] = alpha*conv_yXL(IMU_buf) + (1-alpha)*ay[i-1];
            az[i] = alpha*conv_zXL(IMU_buf) + (1-alpha)*az[i-1];
            gx[i] = alpha*conv_xG(IMU_buf) + (1-alpha)*gx[i-1];
            gy[i] = alpha*conv_yG(IMU_buf) + (1-alpha)*gy[i-1];
            gz[i] = alpha*conv_zG(IMU_buf) + (1-alpha)*gz[i-1];
